...
interface Gif {
	url: string; // mp4-less gif url (fixed height)
	page: string; // giphy page for attribution
	title: string;
}
...
async function search(q: string): Promise<Gif[]> {
	...
	try {
		...
		const gifs = body.data.map((g) => ({ url: g.images.fixed_height.url, page: g.url, title: g.title }));
