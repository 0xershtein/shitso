...
Tweet `@giveashit_bot @handle 💩` (any of the ten emojis), or reply to someone's tweet with just `@giveashit_bot 💩`. The tweet's author is the voter, no login needed; the bot replies with the score and a link. Poller: `/api/cron/xbot`, called every 5 minutes by GitHub Actions.
